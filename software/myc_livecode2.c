...
5-micro/ADC mushroom in -SOFT: GATE, *gate in adc. livecode ADC, cpu=9*
...
void main(void)
{
  ...
  unsigned char (*instructionsetbf[])(unsigned char* cells, unsigned char IP) = {bfinc,bfdec,bfincm,bfdecm,bfoutp,bfin,bfbrac1,bfbrac2,dirout, fetoff, feton}; // 8 +1 +2 =11
  ...
  while(1){
	...
	//        if ((counter%(1+cpuspeed))==0){ // ??? This is where we do speed!
	  ...
	  case 2:
	    instruction=cells[instructionp];
	    instructionp=(*instructionsetbf[instruction%13]) (cells, instructionp);
	    insdir=dir;
	    break;
	  case 3:
	    instruction=cells[instructionp];
